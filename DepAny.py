...
class Negi():
    ...
    def __init__(self):
        ...
        # pixel
        self.y_min , self.y_max = 10 , 280
        self.x_min , self.x_max = 280 , 400
        self.yc_min = self.yc_max = self.xc_min = self.xc_max = None
        self.Cam_to_Plate , self.Top_to_Plate= 0.555 , 0.170
    ...
    def TimeDir(self):
        date = self.time.strftime("%Y\\%m\\%m%d\\%H%M%S")
    ...
    def Capture(self,Cam_number):
        ...
        if ret:
            ...
            self.TimeDir()
            ext = ".jpg"
            cv2.imwrite(os.path.join(self.indir, self.time.strftime("%H%M%S") + ext), self.cropped_frame)
    ...
    def Edge(self,subject):
        ...
        for filename in tqdm(self.filenames,disable = True):
            ...
            if coords.size > 0:
                ...
                self.yc_max, self.xc_max = coords.max(axis=0) - (110, 10)

...
def DepAny():
    if __name__ == '__main__':
        ...
        for filename in tqdm(filenames):
            ...
            Cam_to_Plate = float(0.50) # User Difined Parameter[m]


            # meter depth tensor
            depth_abs = (depth - depth.min()) / (depth.max() - depth.min()) *Cam_to_Plate

            abs_max = Cam_to_Plate -depth_abs.min().item()
            abs_min = Cam_to_Plate -depth_abs.max().item()
            abs_mean = Cam_to_Plate -depth_abs.mean().item()
            abs_median = Cam_to_Plate -depth_abs.median().item()
            ...
            calc_max = depth_calc.max().item()
            calc_min = depth_calc.min().item()
            calc_median = depth_calc.median().item()
            calc_mean = depth_calc.median().item()
